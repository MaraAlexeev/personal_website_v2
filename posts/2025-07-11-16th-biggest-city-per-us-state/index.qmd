...
```{r}
  ranked_cities_by_state_all <- local_data %>%
    ...
    col.names = c("State Name", "Place Name", "2020 Census Population"),
    format.args = list(big.mark = ",")
  )
    
    ranked_cities_by_state_by_pop <-ranked_cities_by_state_all %>% arrange(desc(ESTIMATESBASE2020))
...
Let's see what happened over the past 5 years in Iowa with the top 25 cities. Did Marshalltown hold on???
```{r}
